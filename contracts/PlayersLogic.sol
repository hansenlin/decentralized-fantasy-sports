...
contract PlayersLogic is ERC721 {
  ...
  function addPlayer(bytes32 _name, Position _position, uint256 _playerId) public {
    require(currentDraftPosition == 80);
    require(balanceOf(msg.sender) < 11);
    require(_position != Position.FA);
    players[_playerId] = Player({ name: _name, position: _position});
    _mint(msg.sender, _playerId);
  }
